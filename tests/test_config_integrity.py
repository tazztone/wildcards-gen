...
from wildcards_gen.core.presets import DATASET_PRESET_OVERRIDES, SMART_PRESETS
        ...
        for ds_name, presets in DATASET_PRESET_OVERRIDES.items():
            for p_name, values in presets.items():
                ...
                self.assertLessEqual(
                    depth,
                    MAX_DEPTH,
                    f"Override {ds_name}:{p_name} depth {depth} > {MAX_DEPTH}",
                )
                self.assertLessEqual(
                    hyp,
                    MAX_HYPONYMS,
                    f"Override {ds_name}:{p_name} hyponyms {hyp} > {MAX_HYPONYMS}",
                )
                self.assertLessEqual(
                    leaf,
                    MAX_LEAF,
                    f"Override {ds_name}:{p_name} leaf {leaf} > {MAX_LEAF}",
                )
                self.assertIn(
                    method,
                    ["eom", "leaf"],
                    f"Override {ds_name}:{p_name} invalid method {method}",
                )
