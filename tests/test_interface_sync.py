...
def test_imagenet_signature_sync(base_kwargs):
    with (
        patch("wildcards_gen.core.datasets.imagenet.generate_imagenet_tree") as mock_gen,
...
def test_tencent_signature_sync(base_kwargs):
    ...
    with (
        patch("wildcards_gen.core.datasets.tencent.generate_tencent_hierarchy") as mock_gen,
        patch("wildcards_gen.gui.save_and_preview", return_value=("path", "preview")),
    ):
        generate_dataset_handler(**base_kwargs)
...
def test_openimages_signature_sync(base_kwargs):
    ...
    with (
        patch("wildcards_gen.core.datasets.openimages.generate_openimages_hierarchy") as mock_gen,
        patch("wildcards_gen.gui.save_and_preview", return_value=("path", "preview")),
    ):
        generate_dataset_handler(**base_kwargs)
