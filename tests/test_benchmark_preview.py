...
from wildcards_gen.core.wordnet import ensure_nltk_data, get_all_descendants
...
def test_fast_preview_perf(mock_wn, sample_hierarchy, mock_arranger_deps):
    ...
    with (
        patch(
            "wildcards_gen.core.datasets.imagenet.ensure_imagenet_1k_data",
            return_value="dummy.json",
        ),
        patch(
            "wildcards_gen.core.datasets.imagenet.load_imagenet_1k_wnids",
            return_value=None,
        ),
        ...
        patch(
            "wildcards_gen.core.arranger.get_cached_embeddings",
