...
def mock_ml_stack():
    ...
    with (
        patch("wildcards_gen.core.linter.load_embedding_model") as mock_load,
...
def test_full_stack_no_collisions(mock_ml_stack, complex_smart_kwargs):
    ...
    with (
        patch(
            "wildcards_gen.core.datasets.tencent.download_tencent_hierarchy",
            return_value="/tmp/mock.csv",
        ),
        patch(
