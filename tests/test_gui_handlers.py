...
class TestGUIHandlers(unittest.TestCase):
    ...
    def test_lint_handler_logic(self):
        ...
        with (
            ...
            mock_lint.return_value = (
                {"issues": [{"path": "root", "outliers": [{"term": "bad", "score": 0.9}]}]},
                MagicMock(),
            )
            res, update, path = gui.lint_handler(mock_file, "qwen3", 0.1)
    ...
    def test_analyze_handler(self, mock_gen, mock_suggest, mock_stats):
        ...
        mock_stats_obj.to_dict.return_value = {
            "avg_branching": 2.0,
            "avg_leaf_size": 5.0,
        }
        ...
        # Call handler
        report, d, h, l, hist, stale = gui.analyze_handler("ImageNet", "root.n.01", 10, "none", True, False, False, [])
