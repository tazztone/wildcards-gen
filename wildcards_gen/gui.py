...
# Local Imports
from wildcards_gen.core.datasets import coco, imagenet, openimages, tencent
...
from .core.presets import DATASET_PRESET_OVERRIDES, SMART_PRESETS
...
def update_ds_filename(
    ...
    if bbox_suffix:
        components.append(bbox_suffix.strip("_"))
...
def update_ds_ui(dataset_name, strategy):
    ...
    return [
        ...
        gr.update(
            interactive=can_use_smart,
            value=new_strategy if not can_use_smart else strategy,
        ),  # ds_strategy
...
def on_dataset_change(dataset_name, strategy):
    ...
    return visibility_updates + [gr.update(value=info_text)]
...
def generate_dataset_handler(
    ...
    semantic_threshold,
    semantic_arrange,
    semantic_arrange_threshold,
    semantic_arrange_min_cluster,
    ...
    try:
        ...
        smart_kwargs: Dict[str, Any] = {"stats": stats}  # Always pass stats if available
        if is_smart:
            smart_kwargs.update(
                {
                    "smart": True,
        ...
        if dataset_name == "ImageNet":
            if not root:
                return (
                    None,
                    "Error: Root synset required for ImageNet (e.g. entity.n.01)",
                )
            ...
            data = cast(Any, imagenet.generate_imagenet_tree(**kwargs))

        elif dataset_name == "COCO":
            progress(0.2, desc="Loading COCO API...")
            data = cast(Any, coco.generate_coco_hierarchy(with_glosses=with_glosses))
            ...
            data = cast(Any, openimages.generate_openimages_hierarchy(**kwargs))
            ...
            data = cast(Any, tencent.generate_tencent_hierarchy(**kwargs))
        ...
        summary_md = "### ✅ Generation Complete\n"
...
def analyze_handler(
    ...
    try:
        ...
        if dataset_name == "ImageNet":
            ...
            return (
                "Analysis not supported for this dataset.",
                4,
                50,
                5,
                history,
        ...
        stats = analyze.compute_dataset_stats(cast(Any, data))
...
def create_handler(topic, model, api_key, output_name):
    if not api_key:
        return (
            None,
            "Error: API Key required for LLM features. Set it in the Settings tab.",
        )
    try:
...
def lint_handler(file_obj, model, threshold, progress=gr.Progress()):
    ...
    try:
        from wildcards_gen.core.linter import clean_structure, lint_file
        ...
        if not issues:
            return (
                "✅ **No outliers detected.** The structure looks semantically consistent!",
                gr.update(visible=False),
                None,
            )
...
def launch_gui(share=False, port=None):
    ...
    with gr.Blocks(title="Wildcards-Gen") as demo:
        ...
        with gr.Tabs():
            ...
            with gr.Tab("📸 CV Datasets"):
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=2, elem_classes=["dataset-config-panel"]):
                                ...
                                with gr.Column(scale=1):
                                    with gr.Accordion("🔍 WordNet Lookup", open=False):
                                        search_in = gr.Textbox(
                                            ...
                                            placeholder="camera...",
                                            show_label=False,
                                            info="Search ImageNet for valid WordNet synsets.",
                                        )
                                        ...
                                        search_results = gr.Radio(
                                            ...
                                            visible=False,
                                            interactive=True,
                                            info="Click a synset to populate the Root Synset field.",
                                        )

                                        search_btn.click(
                                            search_wordnet,
                                            inputs=[search_in],
                                            outputs=[search_results, search_msg],
                                        )
                                        search_in.submit(
                                            search_wordnet,
                                            inputs=[search_in],
                        ...
                        ds_depth = gr.Slider(
                            ...
                            step=1,
                            label="Max Generation Depth",
                            info="Limit recursion depth to prevent massive output files.",
                        )

                        # Smart Tuning
                        with gr.Group(visible=True) as smart_tuning_group:
                            gr.Markdown("**Smart Tuning**")
                            ds_smart_preset = gr.Radio(
                                list(SMART_PRESETS.keys()),
                                label="Preset",
                                value="Balanced",
                                info="Quick configurations for semantic pruning aggressiveness.",
                            )
                            ...
                            with gr.Accordion("Fine-Tuning", open=False):
                                ds_min_depth = gr.Slider(
                                    ...
                                    value=4,
                                    step=1,
                                    label="Significance Depth",
                                    info="Keep all categories shallower than this level regardless of descendant count (preserves top-level structure).",
                                )
                                ds_min_hyponyms = gr.Slider(
                                    ...
                                    value=50,
                                    step=10,
                                    label="Flattening Threshold",
                                    info="Merge category into a leaf list if it has fewer than X descendants total.",
                                )
                                ds_min_leaf = gr.Slider(
                                    ...
                                    step=1,
                                    label="Min Leaf Size",
                                    info="If a resulting list has fewer than X items, merge them into the parent node.",
                                )
                                ds_merge_orphans = gr.Checkbox(
                                    label="Merge Orphans",
                                    value=True,
                                    info="Bubble up small lists into the parent's 'misc' key instead of keeping them as separate categories.",
                                )

                            with gr.Accordion("Semantic Cleaning", open=False):
                                ds_semantic_clean = gr.Checkbox(
                                    label="Enable Cleaning",
                                    value=False,
                                    info="Use embeddings to remove items that don't belong in their category (requires local model).",
                                )
                                ds_semantic_model = gr.Dropdown(
                                    ["minilm", "mpnet", "qwen3"],
                                    value="minilm",
                                    label="Model",
                                    info="Embedding model for semantic analysis (Local: minilm/mpnet, Cloud: qwen3).",
                                )
                                ds_semantic_threshold = gr.Slider(
                                    0.01,
                                    1.0,
                                    value=0.1,
                                    step=0.01,
                                    label="Threshold",
                                    info="Higher = more aggressive cleaning (removes more potential outliers).",
                                )

                                gr.Markdown("---")
                                ds_semantic_arrangement = gr.Checkbox(
                                    label="Enable Arrangement",
                                    value=False,
                                    info="Automatically group flat lists into semantic sub-categories.",
                                )

                                with gr.Row():
                                    ds_arrange_threshold = gr.Slider(
                                        label="Quality",
                                        minimum=0.0,
                                        maximum=1.0,
                                        value=0.1,
                                        step=0.05,
                                        info="Min probability required for a cluster.",
                                    )
                                    ds_arrange_min_cluster = gr.Slider(
                                        label="Min Cluster",
                                        minimum=2,
                                        maximum=20,
                                        value=5,
                                        step=1,
                                        info="Minimum items required to form a new automated sub-category.",
                                    )

                                ds_arrange_method = gr.Dropdown(
                                    ["eom", "leaf"],
                                    value="eom",
                                    label="Cluster Method",
                                    info="'leaf' finds smaller, more specific groups; 'eom' finds stable clusters.",
                                )
                                ds_debug_arrangement = gr.Checkbox(
                                    label="Debug Logs",
                                    value=False,
                                    info="Print detailed clustering stats to the terminal.",
                                )

                            with gr.Accordion("Deep Tuning", open=False):
                                with gr.Row():
                                    ds_umap_neighbors = gr.Slider(
                                        2,
                                        50,
                                        value=15,
                                        step=1,
                                        label="UMAP Neighbors",
                                        info="Balances local vs global structure. Lower values focus on very tight details.",
                                    )
                                    ds_umap_dist = gr.Slider(
                                        0.0,
                                        1.0,
                                        value=0.1,
                                        step=0.01,
                                        label="UMAP Min Dist",
                                        info="Determines how tight UMAP packs points. Lower = tighter clusters.",
                                    )
                                with gr.Row():
                                    ds_arr_samples = gr.Slider(
                                        1,
                                        20,
                                        value=5,
                                        step=1,
                                        label="Min Samples",
                                        info="HDBSCAN min samples. Higher = more points labeled as noise.",
                                    )
                                    ds_orphans_template = gr.Textbox(
                                        label="Orphan Key",
                                        value="misc",
                                        placeholder="misc",
                                        info="Label for miscellaneous items (e.g., 'misc' or 'others').",
                                    )
                        ...
                        with gr.Accordion("Advanced Filters", open=False, visible=True) as adv_filter_group:
                            with gr.Row():
                                ds_filter = gr.Dropdown(
                                    ...
                                    label="Sub-Filter",
                                    value="none",
                                    info="Restrict results to specific ImageNet subsets.",
                                )
                                ds_strict = gr.Checkbox(
                                    label="Strict Lexical Match",
                                    value=True,
                                    info="Only include items that exactly match the WordNet synset names.",
                                )
                            with gr.Row():
                                ds_blacklist = gr.Checkbox(
                                    label="Hide Abstract Concepts",
                                    value=False,
                                    info='Filter out purely structural nodes (e.g., "abstraction", "entity").',
                                )
                                ds_bbox_only = gr.Checkbox(
                                    label="Legacy BBox Mode",
                                    value=False,
                                    visible=False,
                                    info="Only include classes with bounding box annotations.",
                                )  # Bound to ds_openimages_group logic but kept here

                            ds_exclude_subtree = gr.Textbox(
                                label="Exclude Subtrees",
                                placeholder="comma-separated wnids",
                                info="Omit entire branches by WNID (e.g., n02084071 for dogs).",
                            )
                            ds_exclude_regex = gr.Textbox(
                                label="Exclude Regex",
                                placeholder="regex patterns",
                                info="Exclude items matching specific name patterns.",
                            )
                    ...
                    with gr.Column(scale=3):
                        ...
                        with gr.Group(elem_classes=["preview-panel"]):
                            with gr.Row():
                                ...
                                ds_out_name = gr.Textbox(
                                    label="Output Filename",
                                    value="skeleton.yaml",
                                    show_label=False,
                                    scale=2,
                                    info="Target YAML filename.",
                                )
                            ...
                            ds_prev = gr.Code(
                                ...
                                lines=35,
                                max_lines=40,
                                elem_classes=["preview-code"],
                            )

                        # Run Controls
                        with gr.Row():
                            ds_fast_preview = gr.Checkbox(
                                label="⚡ Fast Preview",
                                value=False,
                                info="Limit to ~500 items.",
                            )
            ...
            with gr.Tab("🧠 AI Assistant"):
                with gr.Tabs():
                    ...
                    with gr.Tab("✨ New Taxonomy"):
                        ...
                        with gr.Row():
                            with gr.Column():
                                cr_topic = gr.Textbox(
                                    label="Topic",
                                    placeholder="e.g. Types of Cyberpunk Augmentations",
                                    info="The domain or subject matter for the new taxonomy.",
                                )
                                cr_out = gr.Textbox(
                                    label="Output Filename",
                        ...
                        cr_btn.click(
                            create_handler,
                            inputs=[cr_topic, model_state, api_key_state, cr_out],
                    ...
                    with gr.Tab("🗂️ Categorize List"):
                        ...
                        with gr.Row():
                            with gr.Column():
                                cat_terms = gr.TextArea(
                                    label="Raw Terms",
                                    placeholder="List of terms (one per line)...",
                                )
                        ...
                        cat_btn.click(
                            categorize_handler,
                            inputs=[cat_terms, model_state, api_key_state, cat_out],
                    ...
                    with gr.Tab("📝 Enrich (Context)"):
                        ...
                        with gr.Row():
                            with gr.Column():
                                en_yaml = gr.TextArea(
                                    label="Existing YAML",
                                    placeholder="Paste .yaml structure...",
                                )
                                en_topic = gr.Textbox(
                                    label="Context / Goal",
            ...
            with gr.Tab("🛡️ Quality Control"):
                ...
                lint_btn.click(
                    lint_handler,
                    inputs=[lint_file, lint_model, lint_threshold],
                    outputs=[lint_output, lint_clean_group, lint_clean_file],
                )
            ...
            with gr.Tab("⚙️ Settings"):
                with gr.Group():
                    ...
                    set_hf_token = gr.Textbox(
                        label="Hugging Face Token",
                        value=initial_hf_token,
                        type="password",
                        info="Required for gated models or to avoid rate limits.",
                    )
                    ...
                    set_save_keys.click(
                        update_keys,
                        inputs=[set_key, set_hf_token],
                        outputs=[api_key_state, api_status],
                    )
                    set_key.submit(
                        update_keys,
                        inputs=[set_key, set_hf_token],
        ...
        for comp in config_inputs:
            comp.change(update_ds_filename, inputs=config_inputs, outputs=[ds_out_name])  # type: ignore[attr-defined]
        ...
        all_gen_inputs = [
            ...
            ds_filter,
            ds_strict,
            ds_blacklist,  # with_glosses=True default
            ds_min_depth,
            ds_min_hyponyms,
            ds_min_leaf,
            ds_merge_orphans,
            ...
            ds_semantic_clean,
            ds_semantic_model,
            ds_semantic_threshold,
            ds_semantic_arrangement,
            ds_arrange_threshold,
            ds_arrange_min_cluster,
            ds_exclude_subtree,
            ds_exclude_regex,
            ds_arrange_method,
            ds_debug_arrangement,
            ds_umap_neighbors,
            ds_umap_dist,
            ds_arr_samples,
            ds_orphans_template,
        ...
        live_preview_triggers = [
            ds_depth.change,
            ds_min_depth.change,
            ds_min_hyponyms.change,
            ds_min_leaf.change,
            ds_semantic_threshold.change,
            ds_arrange_threshold.change,
            ds_arrange_min_cluster.change,
            ds_merge_orphans.change,
            ds_semantic_clean.change,
            ds_semantic_arrangement.change,
            ds_umap_neighbors.change,
            ds_umap_dist.change,
            ds_arr_samples.change,
        ]
        ...
        ds_smart_preset.change(
            apply_smart_preset,
            inputs=[ds_smart_preset, ds_name],
            outputs=[
                ds_min_depth,
                ds_min_hyponyms,
                ds_min_leaf,
