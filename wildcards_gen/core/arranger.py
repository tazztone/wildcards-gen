...
_MEM_CACHE: Dict[str, Any] = {}  # LRU/Process-local cache
...
def _generate_descriptive_name(
    ...
    """
    metadata = {"wnid": None, "source": "fallback", "examples": cluster_terms[:3]}
    ...
    def is_tautology(candidate: Optional[str]) -> bool:
        if not candidate or not p_norm:
            return False
...
_UMAP_CACHE: Dict[Tuple[str, int, float, int], np.ndarray] = {}
...
def _arrange_single_pass(
    ...
    **kwargs,
    ...
    # Sort for stability
    sorted_labels = sorted(clusters.keys(), key=lambda label: len(clusters[label]), reverse=True)
    ...
    for label in sorted_labels:
        ...
        base_name, meta = _generate_descriptive_name(
            lca_name, cluster_embs, cluster_items, parent_context=parent_context
        )
...
def arrange_list(
    ...
    **kwargs,
    ...
    if not terms or len(terms) < 3:
        return ({}, terms, {}, {}) if return_metadata or return_stats else ({}, terms, None, None)

    if not check_dependencies():
        return (
            ({}, terms, {"error": "missing_dependencies"}, {})
            if return_metadata or return_stats
            else ({}, terms, None, None)
        )
    ...
    if len(embeddings) == 0:
        return (
            ({}, terms, {"error": "no_embeddings"}, {}) if return_metadata or return_stats else ({}, terms, None, None)
        )
    ...
    # Consolidate Stats
    full_stats = {"pass_1": stats_1, "pass_2": stats_2} if return_stats else None

    # Build return tuple based on flags
    # We always return 4 values to match type hint, using None for missing optional data
    return (
        final_groups,
        final_leftovers,
        full_stats if return_stats else None,
        final_metadata if return_metadata else None,
    )
...
def arrange_hierarchy(
    ...
    **kwargs,
    ...
    groups, leftovers, _, _ = arrange_list(terms, return_stats=False, return_metadata=False, context=context, **kwargs)
