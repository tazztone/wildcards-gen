...
def _get_cached_synset_tree() -> Tuple[Dict[str, Any], Dict[str, Any]]:
    ...
    synset_to_labels = {}
    for _label_id, name in id_to_name.items():
    ...
    for _wnid, data in synset_to_labels.items():
        synset = data["synset"]
        labels = data["labels"]
        if synset:
...
def build_taxonomy_tree_from_synsets(
    wnid,
    synset_tree,
    depth,
    max_depth,
    ...
    if not children or depth >= max_depth:
        ...
        return TaxonomyNode(
            name=name,
            children=[],
            items=sorted(list(set(labels)), key=str.casefold),
            metadata={
                "instruction": instruction,
                "synset": synset,
                "depth": depth,
                "is_root": node_data["parent"] is None,
            },
        )
    ...
    if node_data["labels"]:
        child_nodes.append(
            TaxonomyNode(
                name=f"Other {name}",
                items=sorted(node_data["labels"], key=str.casefold),
            )
        )
...
def build_taxonomy_tree_from_json(
    node,
    id_to_name,
    depth,
    max_depth,
