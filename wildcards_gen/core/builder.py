...
from .smart import (
    SmartConfig,
    TraversalBudget,
    apply_semantic_arrangement,
    apply_semantic_cleaning,
    should_prune_node,
)
...
class HierarchyBuilder:
    ...
    def build(self, root: TaxonomyNode) -> CommentedMap:
        ...
        return cast(CommentedMap, shaped)

    def _prune_and_collect(
        self, node: TaxonomyNode, config: Optional[SmartConfig] = None
    ) -> Tuple[Optional[TaxonomyNode], List[str]]:
        """
        ...
        if should_flatten:
            ...
            if current_config.enabled and current_config.semantic_arrangement:
                arranged, leftovers, _ = apply_semantic_arrangement(
                    items, current_config, stats=self.stats, context=name
                )
        ...
        if collected_orphans and current_config.enabled and current_config.merge_orphans:
            ...
            if collected_orphans:
                ...
                if existing_misc:
                    existing_misc.items = sorted(
                        list(set(existing_misc.items + collected_orphans)),
                        key=str.casefold,
                    )
                else:
        ...
        return TaxonomyNode(
            name=name, children=processed_children, items=node.items, metadata=metadata
        ), collected_orphans
            ...
            if instruction:
                try:
                    res.yaml_add_eol_comment(
                        config.instruction_template.format(gloss=instruction),
                        child.name,
                    )
                except Exception:
                    pass
