...
class BatchProcessor:
    ...
    def _parse_job_entry(self, entry: Dict, output_root: str) -> JobConfig:
        ...
        return JobConfig(
            ...
            analyze=entry.get("analyze", self.global_config.get("analyze", False)),
        )
    ...
    def _expand_matrix(self, matrix: Dict, output_root: str) -> List[JobConfig]:
        ...
        for combination in itertools.product(*values_list):
            combo_params = dict(zip(keys, combination, strict=True))
            ...
            expanded_jobs.append(
                JobConfig(
                    ...
                    params=final_params,
                    output_path=out_path,
                    analyze=matrix.get("analyze", self.global_config.get("analyze", False)),
                )
            )
    ...
    try:
        ...
        if job.dataset == "tencent":
            hierarchy = generate_tencent_hierarchy(
                max_depth=p.get("depth", 10),
                with_glosses=not p.get("no_glosses", False),
                smart=p.get("smart", False),
                min_significance_depth=p.get("min_depth", 6),
                min_hyponyms=p.get("min_hyponyms", 10),
                min_leaf_size=p.get("min_leaf_size", 5),
                merge_orphans=p.get("merge_orphans", False),
                semantic_cleanup=p.get("semantic_clean", False),
                semantic_threshold=p.get("semantic_threshold", 0.1),
                semantic_arrangement=p.get("semantic_arrange", False),
                semantic_arrangement_threshold=p.get("semantic_arrange_threshold", 0.15),
                semantic_arrangement_min_cluster=p.get("semantic_arrange_min_cluster", 5),
                stats=stats,
                preview_limit=p.get("preview_limit"),
                smart_overrides=p.get("smart_overrides", None),
            )
        elif job.dataset == "imagenet":
            hierarchy = generate_imagenet_tree(
                root_synset_str=p.get("root", "entity.n.01"),
                max_depth=p.get("depth", 10),
                smart=p.get("smart", False),
                min_significance_depth=p.get("min_depth", 6),
                min_hyponyms=p.get("min_hyponyms", 10),
            ...
            hierarchy = generate_openimages_hierarchy(
                max_depth=p.get("depth", 10),
                smart=p.get("smart", False),
                bbox_only=p.get("bbox_only", False),
                min_significance_depth=p.get("min_depth", 6),
                min_hyponyms=p.get("min_hyponyms", 10),
                min_leaf_size=p.get("min_leaf_size", 5),
                merge_orphans=p.get("merge_orphans", False),
